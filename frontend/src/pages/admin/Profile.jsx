...
const AdminProfile = () => {
  const [form, setForm] = useState({
    ...
    domains: '', social: { linkedin: '', github: '', portfolio: '', twitter: '', instagram: '' }
  });
  ...
  useEffect(() => {
    profileAPI.getMine()
      .then(res => {
        ...
        setCurrent(p);
        setForm({
          name: p.name || '', title: p.title || '', about: p.about || '',
          email: p.email || '', phone: p.phone || '', location: p.location || '',
          domains: p.domains?.join(', ') || '',
          social: { linkedin: p.social?.linkedin || '', github: p.social?.github || '',
            portfolio: p.social?.portfolio || '', twitter: p.social?.twitter || '', instagram: p.social?.instagram || '' }
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries(form).forEach(([k, v]) => {
        if (k === 'social') {
          Object.entries(v).forEach(([sk, sv]) => fd.append(`social[${sk}]`, sv));
        } else if (k === 'domains') {
    ...
    <div className="max-w-2xl">
      ...
      {current?.profileImage && (
        <div className="mb-6 flex items-center gap-4">
          <img src={`${BASE_URL}${current.profileImage}`} alt="Profile"
      ...
      <form onSubmit={handleSubmit} className="space-y-5 card">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <Field label="Professional Title *">
            <input className="input" value={form.title} onChange={e => setForm({...form, title: e.target.value})} required placeholder="Full Stack Developer" />
          </Field>
        </div>

        <Field label="About Me *">
          <textarea className="input resize-y min-h-[120px]" value={form.about} onChange={e => setForm({...form, about: e.target.value})} required placeholder="Tell us about yourself..." rows={4} />
        </Field>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <Field label="Location">
            <input className="input" value={form.location} onChange={e => setForm({...form, location: e.target.value})} placeholder="New York, USA" />
          </Field>
        </div>

        <Field label="Interested Domains">
          <input className="input" value={form.domains} onChange={e => setForm({...form, domains: e.target.value})} placeholder="Web Dev, ML, Cloud (comma-separated)" />
        </Field>

        <div>
          <p className="label mb-3">Social Links</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {['linkedin','github','portfolio','twitter','instagram'].map(s => (
              <div key={s}>
                <label className="text-xs text-gray-500 capitalize block mb-1">{s}</label>
                <input className="input text-sm" value={form.social[s]} placeholder={`https://${s}.com/…`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Profile Photo">
            <input type="file" accept="image/*" className="input py-2 text-sm"
              onChange={e => setFiles({...files, profileImage: e.target.files[0]})} />
          </Field>
